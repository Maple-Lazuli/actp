...
def main(smac, dmac, sip, dip, device):
    # packet = create_eth_header_for_arp(smac) + create_arp_request_payload(smac, sip, dip)
    # cno.rawsend(packet, dev=device)

    packet = create_eth_header_for_arp(smac) + create_arp_response_payload(smac, dmac, sip, dip)
    cno.rawsend(packet, dev=device)
...
if __name__== "__main__":
    ...
    main(args.source_mac, args.dest_mac, args.source_ip, args.dest_ip, args.interface)
